...
public class ChatHub : Hub
{
    ...
    public async Task SendMessage(string receiverId, string content)
    {
        ...
        // Use scoped service to send message
        using var scope = _serviceProvider.CreateScope();
        var messagingService = scope.ServiceProvider.GetRequiredService<IMessagingService>();

        var result = await messagingService.SendMessageAsync(senderId, new SendMessageDto
        {
    ...
    public async Task MarkAsRead(int conversationId)
    {
        ...
        await messagingService.MarkMessagesAsReadAsync(conversationId, userId);
    }
